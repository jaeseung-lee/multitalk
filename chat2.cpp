...
Chat::Chat(string newSend,string newReceive,string newChatting,string newTime){
...
void chatting(ChatRoom room) {
    ...
    cout << "이 방에는 " << Users[me].getName() << ", " << other << "님이 속해있습니다." << endl;

    for(int j=0;j<Chats.size();j++){
        // 받은사람이 나, 보낸사람이 상대방
        if ((Chats[j].getReceive()==other) && (Chats[j].getSend()==Users[me].getName())){
            cout << Chats[j].getTime() << " >> " << Chats[j].getChatting() << endl;
        }
        // 받은사람이 상대방, 보낸사람이 나
        if ((Chats[j].getReceive()==Users[me].getName()) && (Chats[j].getSend()==other)){
            cout << Chats[j].getTime() << " << " << Chats[j].getChatting() << endl;
        }
    }
